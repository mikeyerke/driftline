    ...
    <section id="settings-section" className="panel salesforce-panel" aria-labelledby="salesforce-title">
      ...
      <div className="salesforce-body">
        ...
        {signedIn && <>
          ...
          {connected && <div className="salesforce-connect-action"><div className="salesforce-action-row"><button className="secondary" type="button" onClick={runHealth} disabled={loading}>{loading ? <><RefreshCw size={14} className="spin" />Reading…</> : <><ShieldCheck size={14} />Run aggregate read probe</>}</button><button className="text-button" type="button" onClick={beginAuthorization} disabled={authorizing}>{authorizing ? "Preparing…" : "Reauthorize read-only"}</button></div>{health && <div className="salesforce-health" role="status"><strong>Read verified</strong>{health.objects?.map((item) => <span key={item.object}>{item.object}: {item.total}</span>)}</div>}</div>}
        </>}
