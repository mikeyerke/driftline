...
export default function ScenarioSimulator({ workflowId }) {
  ...
  return (
    <section className="panel scenario-panel" aria-labelledby="scenario-title">
      <header className="panel-header"><div><h2 id="scenario-title"><GitBranch size={17} />Counterfactual decision preview</h2><span className="live-label">No writes</span></div><span className="muted">Compare before approval</span></header>
      {loading && <p className="multimodal-empty"><LoaderCircle size={15} className="spin" />Calculating bounded outcomes…</p>}
