...
export default function CounterfactualCompare({ options, recommendedId, selectedId, onSelect }) {
  ...
  return (
    <section className="decision-room-section counterfactual-section" aria-labelledby="counterfactual-title">
      <header className="decision-room-section-header"><div><span className="decision-room-kicker">Counterfactual decision</span><h3 id="counterfactual-title">Compare what happens next</h3></div><span className="bounded-label">No writes</span></header>
      ...
      {selected && <div className="counterfactual-detail">
        <div className="counterfactual-main"><span className="decision-room-kicker">Expected outcome</span><h4>{selected.summary}</h4><p>{selected.expected_outcome}</p><div className="affected-segments">{selected.affected_segments.map((segment) => <span key={segment}>{segment.replaceAll("_", " ")}</span>)}</div></div>
        <dl>
