...
runtime-code release. Commit `604a353826aba55b309e2084a61140cbef767de8`
...
run `32642860266`. Cloud Build `b8ad1827-c8e5-42d0-99dd-0745e67d5979`
deployed Cloud Run revision `driftline-00273-kdl` at 100% traffic. The
...
- `scripts/verify_live_agent.sh` passed with job `job-3e39a50a6a83`, workflow
  `0cd726dd-aedf-4829-ac5e-4226ae989ef6`, `needs_approval`, `public_source`,
  ...
  `eval-3c87d36b2a85` at 100% / stable.
- `scripts/verify_public_approval_undo.sh` passed with job
  `job-ff85035948f4`, workflow `9c8f1bb6-37c7-4f91-bcf6-fb73aeb4290e`:
...
- `scripts/verify_production.sh` passed with Firestore, Cloud Tasks,
  ...
  A fresh manual Scheduler run at `2026-08-23T13:41:30.625928Z` returned HTTP
  200 on `/api/scheduler/tick` at revision `driftline-00273-kdl`.
...
Salesforce anonymous readiness remains `oauth_ready` / `awaiting_authorization`,
while signed tenant `driftline-demo` remains `reauthorization_required` after
Salesforce rejected its stored refresh token. `external_read=false` and
`aggregate_read_verified=false`; no object totals are claimed. Public
monitoring remains bounded to five pinned fixtures plus exact
operator-registered URLs capped at 25 per tenant; it is not universal
crawling.
...
The current public value-proof window contains 12 source observations and 10
...
- The Settings surface exposes the tenant-scoped Salesforce OAuth handoff,
  ...
  remain unverified until a fresh owner consent succeeds. The bounded probe
  state is durable across page reloads; no CRM records or credential values are
  persisted in that health metadata.
...
- At `2026-08-23T00:16Z`, the signed **Refresh context** read verified the
  tenant-scoped connector boundaries with aggregate-only results: Jira `19`
  open issues, Confluence `7` pages in scope, Slack `42` recent messages, and
  ...
  credential values.
- The signed tenant workflow `c14ed5d6-fecb-4fc3-a40b-8567d5629ce9` reached the
  deterministic approval gate through Google ADK/Gemini 3.5 Flash, then a named
  human approval reactivated the idempotent Jira, Confluence, and Slack handoff
  markers. GitHub was intentionally not selected because it was not in this
  workflow's impact map. The same named human reopened the decision; the three
  mapped connector statuses returned `reversed`, and the browser showed no
...
## 2026-08-22 audited source lifecycle code release (included in current image)

- Commit `210fc9ebb71c7b42aad2735702a6297f1d957890` introduced the code now
  included in the current Cloud Run image. The release adds the
...
- Local proof: 282 backend tests, Ruff, frontend production build, frontend
  ...
  `32557959004` and the code-release Cloud Build `3e8e48b3-3db2-4136-984f-2a275f05a2dc` also
  passed.
...
- The owner-completed callback at `2026-08-21T22:39:18Z` is verified in
  ...
  `2026-08-21T23:41:02Z` returned Salesforce `invalid_grant`; this means the
